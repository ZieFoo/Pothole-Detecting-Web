...
@app.route('/viewer')
def viewer():
    return render_template('viewerv2.html')

@app.route('/map')
def pothole_map():
    return render_template('pothole_mapv2.html')
...
def log_pothole():
    try:
        ...
        screenshot_data  = data.get('screenshot_annotated') or data.get('screenshot')
        ...
        screenshot_live_data = data.get('screenshot_live')
        if screenshot_live_data and data.get('bounding_boxes'):
            try:
                img_data = screenshot_live_data
                if ',' in img_data:
                    img_data = img_data.split(',')[1]
                ...
                for idx, box in enumerate(data.get('bounding_boxes', [])):
                    try:
                        x1, y1, x2, y2 = box

                        padding = 20
                        h,w = img_live.shape[:2]
                        x1_crop = max(0, x1 - padding)
                        y1_crop = max(0, y1 - padding)
                        x2_crop = min(w, x2 + padding)
                        y2_crop = min(h, y2 + padding)

                        cropped = img_live[y1_crop:y2_crop, x1_crop:x2_crop]

                        crop_filename = f"pothole_{timestamp}_box{idx+1}.jpg"
                        crop_path = os.path.join(SCREENSHOTS_DIR, crop_filename)
                        cv2.imwrite(crop_path, cropped)
                        individual_screenshots.append(crop_filename)
                        print(f"Individual box screenshot saved: {crop_filename}")
                    except Exception as e:
                        print(f"Error saving box {idx} screenshot: {e}")
            except Exception as e:
                print(f"Error processing live screenshot: {e}")

        bounding_boxes_with_validation = []
        for idx, box in enumerate(data.get('bounding_boxes', [])):
            box_data = {
                "coords": box,
                "validated": None
            }
            if idx < len(individual_screenshots):
                box_data["screenshot"] = individual_screenshots[idx]
            bounding_boxes_with_validation.append(box_data)
        ...
        log_entry = {
            ...
            "bounding_boxes": bounding_boxes_with_validation,
        ...
        return jsonify({"status": "logged", "entry": log_entry, "count": len(bounding_boxes_with_validation)})
    except Exception as e:
        ...
        if os.path.exists(POTHOLE_LOG_FILE):
            ...
            for detection in detections:
                if detection.get('id') == pothole_id:
                    if 'bounding_boxes' in detection and 0 <= box_index < len(detection['bounding_boxes']):
                        detection['bounding_boxes'][box_index]['validated'] = is_valid
                        print(f"Validated box {box_index} of detection {pothole_id} as {is_valid}")
                    break
                    
            with open(POTHOLE_LOG_FILE, "w") as f:
                json.dump(detections, f, indent=2)

            return jsonify({"status": "success", "id": pothole_id, "box_index": box_index, "validated": is_valid})
        else:
